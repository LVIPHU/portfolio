      ...
      <KBarResults
        ...
        onRender={({ item, active }) => (
          <div>
            {typeof item === 'string' ? (
              <div className='pt-3'>
                <div className='block border-t border-gray-100 px-4 pb-2 pt-6 text-xs font-semibold uppercase text-amber-500 dark:border-gray-800'>
                  {item}
                </div>
              </div>
            ) : (
              <div
                className={`flex cursor-pointer justify-between px-4 py-2 ${
                  active ? 'bg-amber-500 text-gray-100' : 'bg-transparent text-gray-700 dark:text-gray-100'
                }`}
              >
