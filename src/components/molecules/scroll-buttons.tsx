...
export function ScrollButtons() {
  ...
  return (
    <div className={cn('fixed bottom-8 right-8 hidden flex-col gap-3', show && 'lg:flex')}>
      <ScrollButton
        ariaLabel='Scroll To Comment'
        onClick={() => document.getElementById('comment')?.scrollIntoView()}
        icon={MessageSquareText}
      />
      <ScrollButton ariaLabel='Scroll To Top' onClick={() => window.scrollTo({ top: 0 })} icon={ChevronsUp} />
    </div>
  )
}
