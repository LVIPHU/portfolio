...
export function TableOfContents({ toc, className }: { toc: TocItem[]; className?: string }) {
  return (
    <details className={cn('space-y-4 [&_.chevron-right]:open:rotate-90', className)} open>
      <summary className='flex cursor-pointer items-center gap-1 marker:content-none'>
        <ChevronRight strokeWidth={1.5} size={20} className='chevron-right rotate-0 transition-transform' />
      ...
      <ul className='flex flex-col space-y-2'>
        {toc.map(({ value, depth, url }) => (
          ...
          >
            <NavigationLink href={url}>{value}</NavigationLink>
          </li>
        ))}
      </ul>
    </details>
